...
import { futureIso, registerUser, seedExam } from './helpers/factories';
...
describe('Appointments (e2e)', () => {
  ...
  describe('POST /appointments', () => {
    ...
    it('rejeita sobreposição com 409', async () => {
      ...
      await user.agent.post('/appointments').send({ examId: exam.id, scheduledAt: at }).expect(201);
  ...
  describe('GET /appointments', () => {
    it('retorna apenas os próprios agendamentos', async () => {
      ...
      const response = await alice.agent.get('/appointments').expect(200);
  ...
  describe('PATCH /appointments/:id/cancel', () => {
    ...
    it('403 quando não é o dono', async () => {
      ...
      await intruder.agent.patch(`/appointments/${created.body.id}/cancel`).expect(403);
    });
    ...
    it('409 se já cancelado', async () => {
      ...
      await user.agent.patch(`/appointments/${created.body.id}/cancel`).expect(200);
      await user.agent.patch(`/appointments/${created.body.id}/cancel`).expect(409);
    });
    ...
    it('409 se o agendamento já é passado', async () => {
      ...
      await ctx.db
        ...
        .where(eq(appointments.id, created.body.id));
    ...
    it('400 para id não-UUID', async () => {
      ...
      await user.agent.patch('/appointments/not-a-uuid/cancel').expect(400);
    });
  });
});
